...
//             console.log(req.data)
//         })   
//        let poop = document.getElementById('repeat-text')
//        poop.textContent = req.data   
// }
...
const epic = 'epic'
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${epic}`).then(
        (req, res) => {
            let  temp = document.querySelector('#repeat-text')
            console.log(req.data)
            temp.textContent = req.data
            console.log(temp)
        }
        )
}
...
//         .then({data} = (req) => console.log(data))
// }

// document.getElementById('query-button').addEventListener('click', queryButton)

let queryParam = 'epic=gamer'
let queryParam2 = 'gamer=epic'
function rqstr(){
    axios.get(`http://localhost:3000/query-test/?${queryParam}`).then(
        (req, res) => {
            console.log(req.data)
        }
    )
}
document.querySelector('#query-button').addEventListener('click', rqstr)
...
/*
    ...
    the system says "you sent an empty query!"

    2: Send more than 1 query on the URL -- what happened? 

    the system logs 'You sent more than 1 query!'
*/
